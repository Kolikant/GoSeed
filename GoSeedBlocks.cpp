...
unordered_map<string, int> hashmap;                  // the hashtable where the second volumes blocks are stored
...
void put_v2_in_hash_table(std::ifstream &fv2)
{
    for (int i = 0; i < 50; i++)
    {
        std::string content;
        std::getline(fv2, content);
...
int main(int argc, char *argv[])
{
    ...
    M_presents = std::stod(std::string(argv[4]));
    epsilon_presents = std::stod(std::string(argv[5]));
    ...
    put_v2_in_hash_table(fv2, hashtable)
