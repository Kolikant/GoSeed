...
void calculate_migration_and_save_solution(GRBVar *blocks_migrated, GRBVar *blocks_replicated, std::vector<bool> blocks_is_in_intersect, GRBVar *files, std::string print_to, double *block_size)
{
    ...
    {
        if (blocks_migrated[i / 2 ].get(GRB_DoubleAttr_X) != 0.0)
        {
            total_blocksMigrated += block_size[i / 2];
            if (blocks_is_in_intersect[i / 2]) {
                total_volume_change_clean += block_size[i / 2];
            } else {
                total_traffic += block_size[i / 2];
            }
        }
    }
    ...
    for (long int i = 0; i < num_of_blocks; i++)
    {
        if (blocks_replicated[i / 2].get(GRB_DoubleAttr_X) != 0.0)
        {
            total_blocksReplicated += block_size[i / 2];
            if (blocks_is_in_intersect[i / 2]) {
                ;
            } else {
                total_volume_change_add += block_size[i / 2];
            }
        }
    }    
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        double *block_size = new double[num_of_blocks / 2 + 1]; //init block size array will be used later on building model's equations and objective function.
        std::fill_n(block_size, num_of_blocks / 2 + 1, 0);

        model.set("Seed", seed.c_str());
        model.set("Threads", number_of_threads.c_str());

        //set the model's variables.
        blocks_migrated = model.addVars(num_of_blocks / 2 + 1, GRB_BINARY);
        blocks_replicated = model.addVars(num_of_blocks / 2 + 1, GRB_BINARY);
        ...
        while (std::getline(f, content))
        {
            ...
            if (splitted_content[0] == "F")
            {
                ...
                for (register int i = 0; i < 2 * number_of_blocks_in_file_line; i += 2) //read block_sn and block_size simultaneously and add constrains to the model.
                {
                    ...
                    block_sn = std::stoi(splitted_content[5 + i]) / 2;
            ...
            else if (splitted_content[0] == "B")
            {
                ...
                block_sn = std::stoi(splitted_content[1]) / 2;
        ...
        for (int i = 0; i < num_of_blocks; i++)
        {
            all_migrated_blocks += blocks_migrated[i / 2] * block_size[i / 2];
            all_replicated_blocks += blocks_replicated[i / 2] * block_size[i / 2];
            if( !blocks_is_in_intersect[i / 2] ) {
                totalTraffic += blocks_migrated[i / 2] * block_size[i / 2] + blocks_replicated[i / 2] * block_size[i / 2];
                totalAdd += blocks_replicated[i / 2] * block_size[i / 2];
            } else {
                totalClean += blocks_migrated[i / 2] * block_size[i / 2];
            }
            total_block_size_Kbytes += block_size[i / 2];
        }
        ...
        block_size = new double[num_of_blocks / 2 + 1];
    ...
    {
        std::cout << "I'll never tell you what the problem was >:)" << std::endl;
    }
