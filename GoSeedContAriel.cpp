...
double actual_T_percentage = UNDEFINED_DOUBLE;            //The % of physical containers we decided to migrate.
...
void calculate_migration_and_save_solution(GRBVar *containers_migrated, GRBVar *files, std::string print_to)
{
    ...
    actual_T_percentage = (actual_M / (double)num_of_containers_after_group) * 100.0;
    solution.close();
}
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        M_containers = num_of_containers_after_group * T_percentage / 100;             //assign the number of bytes to migrate
        epsilon_containers = num_of_containers_after_group * MM_percentage / 100; //assign the epsilon in bytes.
